...
;; Package-Commit: cdd83ff9965d8df7aaf95e5d7751d2feabbd5493
...
will be deleted and recreated."
  ...
  (let* (item
    ...
    (message "output-directory=%s" output-directory)
    ...
    (message "=====4 final-pkg-list=%s" final-pkg-list)
    (when (and (> (length final-pkg-list) 0)
               output-directory
               (file-directory-p output-directory))

      (message "=====5 dirs=%s" dirs)
      (dolist (dir dirs)
